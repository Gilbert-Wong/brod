...
-record(state,
        ...
        , coordinator :: ?undef | brod:endpoint()
          %% The connection pid to the group coordinator broker.
          %% This connection is a meta-data connection managed by `brod_client'
          %% so the `heartbeat' and `offset_fetch' requests will not have to
          %% share tcp link with `produce' or `fetch' requests which can be
          %% potentially very large in size.
        , connection :: ?undef | kpro:connection()
          %% Monitor reference of the connection to group coordinator broker
...
discover_coordinator(#state{ client     = Client
  ...
  Connection1 =
    ?ESCALATE(brod_client:get_group_coordinator_connection(Client, GroupId)),
  {Connection, Mref} =
    case Connection1 =:= Connection0 of
      true ->
        {Connection0, Mref0};
      false ->
        is_reference(Mref0) andalso erlang:demonitor(Mref0, [flush]),
        Mref1 = erlang:monitor(process, Connection1),
        {Connection1, Mref1}
    end,
  {ok, State#state{connection = Connection, conn_mref = Mref}}.
...
join_group(#state{ groupId                 = GroupId
  ...
  #kpro_rsp{ api = join_group
           , vsn = Vsn
           , msg = RspBody
           } = send_sync(Connection, Req, SessionTimeout),
...
sync_group(#state{ groupId       = GroupId
  ...
  #kpro_rsp{ api = sync_group
           , vsn = Vsn
           , msg = RspBody
           } = send_sync(Connection, SyncReq),
  ?ESCALATE_EC(kpro:find(error_code, RspBody)),
...
do_commit_offsets_(#state{ groupId                  = GroupId
  ...
  #kpro_rsp{ api = offset_commit
           , vsn = Vsn
           , msg = RspBody
           } = send_sync(Connection, Req),
...
get_committed_offsets(#state{ offset_commit_policy = commit_to_kafka_v2
  ...
  Req = brod_kafka_request:offset_fetch_request(Connection, GroupId,
                                                GrouppedPartitions),
...
send_sync(Connection, Request, Timeout) ->
  ?ESCALATE(kpro:request_sync(Connection, Request, Timeout)).
