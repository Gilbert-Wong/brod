...
handle_batches(?undef, [], #state{begin_offset = LastOffset} = State0) ->
  %% A meta-batch. e.g. transactional session ID initialization batch
...
handle_batches(_Header, ?incomplete_batch(Size),
  ...
  State = maybe_send_fetch_request(State1),
  {noreply, State};
handle_batches(_Header, [], State0) ->
  %% no batch found, poll again (maybe after a delay)
...
%% @private Init payload connection regardless of subscriber state.
...
%% @private Send a ?INIT_CONNECTION delayed loopback message to re-init.
