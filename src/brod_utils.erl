...
-spec fetch([endpoint()], topic(), partition(), offset(),
            ...
            conn_config()) -> {ok, [brod:message()]} | {error, any()}.
fetch(Hosts, Topic, Partition, Offset, WaitTime,
      MinBytes, MaxBytes, ConnCfg) ->
  with_conn(
    kpro:connect_partition_leader(Hosts, ConnCfg, Topic, Partition),
    fun(Pid) ->
...
make_fetch_fun(Conn, Topic, Partition, WaitTime, MinBytes, MaxBytes) ->
...
%% 1. get connection by calling
%%    `brod_client:get_group_coordinator_connection'
...
fetch_committed_offsets(Client, GroupId, Topics) ->
  case brod_client:get_group_coordinator_connection(Client, GroupId) of
    {ok, Conn} -> do_fetch_committed_offsets(Conn, GroupId, Topics);
...
-spec parse_fetch_rsp(kpro:vsn(), kpro:struct()) -> map().
parse_fetch_rsp(APIVsn, Msg) ->
  ...
  {Header, Batches, EC2} =
    case kpro:find(responses, Msg) of
      ...
      _ ->
        ...
        {Header0, kpro:decode_batches(APIVsn, Records), ECx}
    end,
...
parse(sync_group, _, Msg) ->
  ...
  ok = throw_error_code([Msg]),
  kpro:find(member_assignment, Msg);
