...
-export([ get_consumer/3
        , get_group_coordinator_connection/2
...
%% @doc Get connection to the group coordinator broker.
-spec get_group_coordinator_connection(client(), group_id()) ->
        {ok, connection()} | {error, any()}.
get_group_coordinator_connection(Client, GroupId) ->
  safe_gen_call(Client, {get_group_coordinator_connection, GroupId}, infinity).
...
handle_info(init, State) ->
  ...
  NewState = State#state{ bootstrap_endpoints = Endpoints
...
handle_call({get_group_coordinator_connection, GroupId}, _From, State) ->
  {Result, NewState} = do_get_group_coordinator_connection(State, GroupId),
  {reply, Result, NewState};
...
terminate(Reason, #state{ client_id     = ClientId
  ...
  Shutdown = fun(#conn{pid = Pid}) -> brod_utils:shutdown_pid(Pid) end,
  lists:foreach(Shutdown, PayloadConns),
  lists:foreach(Shutdown, MetaConns).
...
do_get_metadata(Topic, #state{ client_id     = ClientId
  ...
  Request = brod_kafka_request:metadata(Conn, [Topic || Topic =/= ?undef]),
...
ensure_metadata_connection(#state{ bootstrap_endpoints = Endpoints
  ...
  {ok, Pid} = kpro:connect_any(Endpoints, ConnConfig),
  {ok, Endpoint} = kpro_connection:get_endpoint(Pid),
  Conn = #conn{endpoint = Endpoint, pid = Pid},
  State#state{meta_conns = [Conn]};
ensure_metadata_connection(State) ->
  State.

%% must be called after ensure_metadata_connection
get_metadata_connection(#state{meta_conns = [#conn{pid = Pid} | _]}) -> Pid.
...
-spec do_get_group_coordinator_connection(state(), group_id()) ->
        {Result, state()} when Result :: {ok, connection()} | {error, any()}.
do_get_group_coordinator_connection(State0, GroupId) ->
  State = ensure_metadata_connection(State0),
  MetaConn = get_metadata_connection(State),
  Timeout = timeout(State),
  case kpro:discover_coordinator(MetaConn, group, GroupId, Timeout) of
    {ok, Endpoint} ->
      case find_conn(Endpoint, State#state.meta_conns) of
        {ok, Pid} ->
          {{ok, Pid}, State};
        {error, not_found} ->
          add_meta_connection(Endpoint, State)
      end;
    {error, Reason} ->
      {{error, Reason}, State}
  end.

-spec add_meta_connection(endpoint(), state()) ->
        {Result, state()} when Result :: {ok, connection()} | {error, any()}.
add_meta_connection({Host, Port} = Endpoint,
                    #state{meta_conns = Conns} = State0) ->
  case do_connect(Host, Port, State0) of
    {ok, Pid} ->
      C = #conn{endpoint = Endpoint, pid = Pid},
      State = State0#state{meta_conns = [C | Conns]},
      {{ok, Pid}, State};
    {error, Reason} ->
      {{error, Reason}, State0}
  end.
...
conn_config(#state{client_id = ClientId, config = Config}) ->
  conn_config(ClientId, Config).

conn_config(ClientId, Config) ->
  F = fun(KeyName, Acc) ->
        case config(KeyName, Config, ?undef) of
          ?undef -> Acc;
          Val -> Acc#{KeyName => Val}
        end
      end,
  Conf = lists:foldl(F, #{}, kpro_connection:all_cfg_keys()),
  Conf#{client_id => ensure_binary(ClientId)}.
...
connect(#state{ client_id = ClientId
  ...
  Conn =
    case do_connect(Host, Port, State) of
      {ok, Pid} ->
      ...
      {error, Reason} ->
        error_logger:error_msg("client ~p failed to connect to ~s:~p~n"
                               "reason:~p",
                               [ClientId, Host, Port, Reason]),
        mark_conn_dead(#conn{endpoint = Endpoint}, Reason)
    end,
  Result = case Conn of
             #conn{pid = P} when is_pid(P) -> {ok, P};
             #conn{pid = ?dead_since(_, R)} -> {error, R}
           end,
  NewConns = lists:keystore(Endpoint, #conn.endpoint, Conns, Conn),
  {Result, State#state{payload_conns = NewConns}}.

do_connect(Host, Port, State) ->
  ConnConfig = conn_config(State),
  kpro_connection:start(Host, Port, ConnConfig).

%% Handle connection pid EXIT event, keep the timestamp.
...
handle_connection_down(#state{meta_conns = Conns} = State, Pid, Reason) ->
  case lists:keytake(Pid, #conn.pid, Conns) of
    {value, _Conn, Rest} ->
      State#state{meta_conns = Rest};
    false ->
      handle_payload_connection_down(State, Pid, Reason)
  end.

handle_payload_connection_down(#state{ payload_conns = Conns
                                     , client_id     = ClientId
                                     } = State, Pid, Reason) ->
  case lists:keytake(Pid, #conn.pid, Conns) of
    {value, #conn{endpoint = {Host, Port}} = Conn, Rest} ->
      ...
      NewConn = mark_conn_dead(Conn, Reason),
...
mark_conn_dead(Conn, Reason) ->
  Conn#conn{pid = ?dead_since(os:timestamp(), Reason)}.
